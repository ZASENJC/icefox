...
class Plugin implements PluginInterface
{
    ...
    public static function config(\Typecho\Widget\Helper\Form $form)
    {
        ...
        // 获取现有友情链接
        $links = self::getLinks();
        $deleteLinkUrl = json_encode(
            \Helper::security()->getIndex('/action/icefox?do=deleteFriendLink'),
        ...
        foreach ($links as $link) {
            ...
            echo '<td><input type="text" name="links[' . $link['id'] . '][name]" value="' . self::escape($link['name']) . '" class="text" required></td>';
            echo '<td><input type="url" name="links[' . $link['id'] . '][url]" value="' . self::escape($link['url']) . '" class="text" required></td>';
            echo '<td><input type="url" name="links[' . $link['id'] . '][avatar]" value="' . self::escape($link['avatar']) . '" class="text"></td>';
            echo '<td><input type="text" name="links[' . $link['id'] . '][description]" value="' . self::escape($link['description']) . '" class="text"></td>';
    ...
    public static function saveLinks()
    {
        ...
        if (isset($_POST['links']) && is_array($_POST['links'])) {
            foreach ($_POST['links'] as $id => $link) {
                $url = self::normalizeHttpUrl($link['url'] ?? '');
                if (empty($link['name']) || $url === '') {
                    continue;
                }
                ...
                $db->query(
                    $db->update($prefix . 'icefox_links')
                        ->rows([
                            'name' => self::normalizeText($link['name'], 100),
                            'url' => $url,
                            'avatar' => self::normalizeHttpUrl($link['avatar'] ?? '', true),
                            'description' => self::normalizeText($link['description'] ?? '', 200),
        ...
        if (isset($_POST['new_link']) && !empty($_POST['new_link']['name']) && !empty($_POST['new_link']['url'])) {
            ...
            $url = self::normalizeHttpUrl($newLink['url']);
            if ($url !== '') {
                $db->query(
                    $db->insert($prefix . 'icefox_links')
                        ->rows([
                            'name' => self::normalizeText($newLink['name'], 100),
                            'url' => $url,
                            'avatar' => self::normalizeHttpUrl($newLink['avatar'] ?? '', true),
                            'description' => self::normalizeText($newLink['description'] ?? '', 200),
    ...
    private static function normalizeHttpUrl($value, $allowEmpty = false)
    {
    ...
    private static function normalizeText($value, $length)
    {
