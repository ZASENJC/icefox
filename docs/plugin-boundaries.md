...
`core/plugin-bridge.php` 仍会直接读取插件的 `icefox_archive` 表，用于兼容现有文章置顶数据。这是当前唯一允许的跨边界数据库访问。

下一步应由插件把置顶状态暴露为 Typecho 文章字段或稳定的服务端挂载点，然后删除这个直接查表的兼容层。在插件改造完成前直接删除它，会让现有置顶标记失效。
