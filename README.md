...
2. **安装配套插件**（必需）
   ...
   旧版文章置顶数据需按分工文档运行一次迁移脚本；新版置顶由主题的 `isTop` 文章字段直接管理和排序。
...
4. 插件返回的相册对象可使用 `id`/`slug`、`name`、`cover`、`tags`、`address`、`visibility`、`isPinned` 和 `photos` 字段；置顶相册会优先显示并在封面左上角显示红色图标，未设置 `cover` 时主题使用 `photos` 的第一张图片作为封面。
...
| `do=saveAlbum` | POST multipart | 新建或编辑相册并上传照片；`isPinned=1/0` 控制置顶状态 |
